...
schema = StructType([
    ...
    StructField("rain", StructType([
        StructField("last_hour", FloatType(), True)
    ]), True),
...
raw_data = spark.readStream.format("socket").option("host", "127.0.0.1").option("port", 9999).load()

weather_data = raw_data.select(from_json(col("value").cast("string"), schema).alias("weather"))

weather_data_expanded = weather_data.selectExpr("weather.coord.lon as lon", 
...
query = weather_data_expanded.writeStream.outputMode("append").format("console").start()
